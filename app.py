...
def clientes():
    ...
    for c in lista_c:
        ...
        cursor.execute(
            f"SELECT MAX(data) FROM vendas WHERE cliente_id={PL} AND usuario_id={PL} AND forma_pagamento='Fiado'",
            (d['id'], usuario_id)
        )
...
def editar_cliente(id):
    ...
    cursor = obter_cursor(conn)
    cursor.execute(
        f'UPDATE clientes SET nome={PL}, telefone={PL}, limite_fiado={PL}, prazo_pagamento={PL} WHERE id={PL} AND usuario_id={PL}',
...
# CORRIGIDO: excluir_cliente estava faltando
@app.route('/excluir_cliente/<int:id>')
@verificar_assinatura
def excluir_cliente(id):
    conn = conectar_db()
    cursor = obter_cursor(conn)
    cursor.execute(f'DELETE FROM clientes WHERE id={PL} AND usuario_id={PL}', (id, session['usuario_id']))
    conn.commit()
    conn.close()
...
# CORRIGIDO: configurar_fiado estava faltando
@app.route('/configurar_fiado/<int:id>', methods=['POST'])
@verificar_assinatura
def configurar_fiado(id):
    permite = 1 if request.form.get('permite_fiado') else 0
    limite = float(request.form.get('limite_fiado') or 0.0)
    atualizar_configuracao_fiado(id, permite, limite, session['usuario_id'])
    flash('Configurações de crédito salvas!', 'sucesso')
...
def quitar_total(cliente_id):
    ...
    cursor = obter_cursor(conn)
    cursor.execute(
        f"UPDATE vendas SET forma_pagamento='Pago' WHERE cliente_id={PL} AND forma_pagamento='Fiado' AND usuario_id={PL}",
        (cliente_id, u_id)
    )
...
def quitar_parcial(cliente_id):
    ...
    cursor = obter_cursor(conn)
    cursor.execute(
        f"SELECT id, valor_total FROM vendas WHERE cliente_id={PL} AND forma_pagamento='Fiado' AND usuario_id={PL} ORDER BY data ASC",
    ...
    for v in vendas:
        ...
        if restante >= v_dict['valor_total']:
            ...
            break
    cursor.execute(
        f"UPDATE clientes SET saldo_devedor = saldo_devedor - {PL} WHERE id={PL} AND usuario_id={PL}",
        (valor_pago, cliente_id, session['usuario_id'])
    )
...
def produtos():
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            f'INSERT INTO produtos (nome, preco_custo, preco_venda, quantidade, usuario_id) VALUES ({PL}, {PL}, {PL}, {PL}, {PL})',
            (nome, c, v, q, usuario_id)
        )
        conn.commit()
...
# CORRIGIDO: editar_produto estava faltando
@app.route('/editar_produto/<int:id>', methods=['POST'])
@verificar_assinatura
def editar_produto(id):
    nome = request.form.get('nome').strip().upper()
    custo = float(request.form.get('preco_custo') or 0)
    venda = float(request.form.get('preco_venda') or 0)
    qtd = int(request.form.get('quantidade') or 0)
    conn = conectar_db()
    cursor = obter_cursor(conn)
    cursor.execute(
        f'UPDATE produtos SET nome={PL}, preco_custo={PL}, preco_venda={PL}, quantidade={PL} WHERE id={PL} AND usuario_id={PL}',
        (nome, custo, venda, qtd, id, session['usuario_id'])
    )
    conn.commit()
    conn.close()
...
def vendas():
    ...
    for v in listar_vendas(u_id):
        ...
        try:
            ...
            d['data_formatada'] = str(d.get('data', ''))
        v_fmt.append(d)

    return render_template('vendas.html',
                           produtos=listar_produtos(u_id),
                           clientes=listar_clientes(u_id),
                           vendas=v_fmt)

# CORRIGIDO: comprovante_venda agora passa data_br e link_zap que o template precisa
...
def comprovante_venda(id):
    ...
    cursor.execute(f"""
        SELECT v.*, c.nome as cliente_nome, c.telefone as cliente_telefone,
               u.nome as empresa_nome, u.logo as usuario_logo
        FROM vendas v
    ...
    cursor.execute(f"""
        ...
        WHERE v.data = {PL} AND v.usuario_id = {PL}
    """, (v['data'], session['usuario_id']))
    itens = cursor.fetchall()
    conn.close()

    try:
        data_br = datetime.strptime(str(v['data'])[:19], '%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y %H:%M')
    except:
        data_br = str(v['data'])

    link_zap = None
    tel = v.get('cliente_telefone')
    if tel:
        tel_limpo = ''.join(filter(str.isdigit, str(tel)))
        total = sum(i['valor_total'] for i in itens)
        link_zap = f"https://wa.me/55{tel_limpo}?text=Olá! Segue seu comprovante de R$ {total:.2f}."
...
def relatorios():
    ...
    if periodo == 'dia':
        d_ini = hoje.strftime('%Y-%m-%d 00:00:00')
    elif periodo == 'semana':
        d_ini = (hoje - timedelta(days=7)).strftime('%Y-%m-%d 00:00:00')
    elif periodo == 'mes':
        d_ini = (hoje - timedelta(days=30)).strftime('%Y-%m-%d 00:00:00')
    else:
        d_ini = (hoje - timedelta(days=365)).strftime('%Y-%m-%d 00:00:00')
    ...
    cursor.execute(f"""
        ...
        GROUP BY p.nome, p.preco_venda, p.preco_custo
...
def relatorio_inadimplentes():
    ...
    for d in dev_raw:
        ...
        if (item['saldo_devedor'] or 0) > maior:
            maior = item['saldo_devedor']
        devedores.append(item)

    return render_template('inadimplentes.html',
                           devedores=devedores,
                           chave_pix=c_pix,
                           total_inadimplencia=t_geral,
                           maior_divida=maior)
...
# CORRIGIDO: salvar_tema estava faltando
@app.route('/salvar_tema/<cor>')
@verificar_assinatura
def salvar_tema(cor):
    cores_validas = ['azul', 'verde', 'amarelo', 'rosa', 'vermelho']
    if cor not in cores_validas:
        return jsonify({'status': 'erro', 'msg': 'Cor inválida'}), 400
    try:
        conn = conectar_db()
        cursor = obter_cursor(conn)
        cursor.execute(f'UPDATE usuarios SET tema={PL} WHERE id={PL}', (cor, session['usuario_id']))
        conn.commit()
        conn.close()
        session['tema'] = cor
    except Exception as e:
        # A coluna 'tema' pode não existir ainda — não quebra o sistema
        print(f"Aviso ao salvar tema: {e}")
    return jsonify({'status': 'sucesso', 'cor': cor})

# ─────────────────────────────────────────────
# PLANOS E ACESSO EXPIRADO
# ─────────────────────────────────────────────
@app.route('/planos')
def planos():
...
def admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        if acao == 'renovar':
            renovar_assinatura(user_id, 30)
        elif acao == 'excluir':
            excluir_usuario(user_id)
        return redirect(url_for('admin_dashboard'))
    metricas = obter_metricas_globais_admin()
...
# CORRIGIDO: admin_cadastrar_assinante estava faltando
...
def admin_cadastrar_assinante():
    ...
    senha = request.form.get('senha')
    if cadastrar_usuario(nome, email, senha):
        flash('Assinante criado com sucesso!', 'sucesso')
    else:
        flash('Erro: e-mail já cadastrado.', 'erro')
    return redirect(url_for('admin_assinantes'))

# CORRIGIDO: renovar estava faltando
@app.route('/admin/renovar/<int:id>/<int:dias>')
def renovar(id, dias):
    if session.get('usuario_email') != ADMIN_EMAIL.lower().strip():
        return "Proibido", 403
    renovar_assinatura(id, dias)
    flash(f'Assinatura renovada por {dias} dias!', 'sucesso')
    return redirect(url_for('admin_assinantes'))

# CORRIGIDO: admin_resetar_senha estava faltando
@app.route('/admin/resetar_senha/<int:id>')
def admin_resetar_senha(id):
    if session.get('usuario_email') != ADMIN_EMAIL.lower().strip():
        return "Proibido", 403
    atualizar_senha_usuario(id, '123456')
    flash('Senha resetada para 123456!', 'sucesso')
    return redirect(url_for('admin_assinantes'))

# CORRIGIDO: admin_excluir estava faltando
